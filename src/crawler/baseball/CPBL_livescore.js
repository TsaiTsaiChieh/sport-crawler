...
async function CPBLpbpInplay(path, totalData, date) {
  ...
  // const betsID = '2020092211235G207';
  const URL = 'http://www.cpbl.com.tw/games/play_by_play.html?&game_type=01&game_id=207&game_date=2020-09-22&pbyear=2020';
  ...
  for (let i = 0; i < awayScore.length; i++) {
    if (awayScore[i] !== '' && homeScore[i] === '') {
      ...
      inningsNow = i + 1;
      halfsNow = '0';
    } else if (awayScore[i] !== '' && homeScore[i] !== '' && awayScore[i + 1] === '') {
      // 現在為 i + 1 局下半
      inningsNow = i + 1;
      halfsNow = '1';
    }
  }
  console.log(inningsNow);
  console.log(halfsNow);
}
